...
public class Soldier
{
	...
	private static void populateAttackSquares() throws GameActionException
	{
		for(int i = 0; i < 7; i++)
		{
			for(int j = 0; j < 7; j++)
			{
				if(grandNeighborArray[i][j] == -100) // if the square is an enemy soldier
				{
					updateGrandArrayAttack(i, j, -1);
				}
				else if(grandNeighborArray[i][j] == 100)
				{
					updateGrandArrayAttack(i, j, 1);
				}
			}
	...
	private static void updateGrandArrayAttack(int i, int j, int team) throws GameActionException
	{
		for(int a = i - 2; a <= i + 2; a++) 
		{
			if(a >= 0 && a <= 6)
			{
				for(int b = j - 2; b <= j + 2; b++)
				{
					if(b >= 0 && b <= 6)
					{
						int tmp = grandNeighborArray[a][b];
						if(tmp != -100 && tmp != 100 && tmp != -1000 && tmp != 1000)
						{
							grandNeighborArray[a][b] += team;
						}
					}
				}
			}
	...
	private static Direction getMove() throws GameActionException
	{
		...
		{
			int[] indexedLocation = locToIndex(rc.getLocation(), rc.getLocation().add(d), 3);
			int attackableRobos = getNumberOfAttackableRobos(indexedLocation);
			rc.setIndicatorString(1, attackableRobos + "");

			goodMoves.add(new MoveInfo(attackableRobos, d, indexedLocation[0], indexedLocation[1], grandNeighborArray[indexedLocation[1]][indexedLocation[1]]));
		}

		Collections.sort(goodMoves);
		return goodMoves.get(0).d;
	}

	private static int getNumberOfAttackableRobos(int[] index)
	{
		int x = index[0];
		int y = index[1];
		int retval = 0;
		...
		for(int i = -1 + x; i < 2 + x; i++)
		{
			...
			{
				if(!(i == 0 && j == 0) && (grandNeighborArray[i][j] == -100 || grandNeighborArray[i][j] == -1000))
				{
					retval++;
				}
			}
	...
	private static void populate7x7neighbors() throws GameActionException
	{
		...
		for(Robot aRobot : closeRobos)
		{
			...
			if(index[0] >= 0 && index[0] <= 6 && index[1] >= 0 && index[1] <= 6)
			{
				...
				{
					array[index[0]][index[1]] = info.team == rc.getTeam() ? 1000 : -1000;
				}
			}
		}
		grandNeighborArray = array;
	}
