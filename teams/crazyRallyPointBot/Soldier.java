package crazyRallyPointBot; // test
...
public class Soldier
{
	...
	public static void soldierCode(RobotController myRC) throws GameActionException
	{
		...
		while(true)
		{
			try
			{
				// returns all enemy robots reasonably close to us (a.k.a that we should move toward and potentially assist our allies in fighting)
				Robot[] enemyRobots = rc.senseNearbyGameObjects(Robot.class,63,rc.getTeam().opponent());
				
				if(rc.readBroadcast(Constants.moveTypeChannel) == Constants.moveTypeRightClick || enemyRobots.length == 0 ) // no enemies are nearby
				{
