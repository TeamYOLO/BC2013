package crazyRallyPointBot;

public class Constants {
	private Constants() { }  // Prevents instantiation

	public static final int campChannel   = 99;
	public static final int campGen = 1;
	public static final int campGenInProduction = 2;
	public static final int campSupplier = 0;
	
	public static final int attackChannel = 100;
	public static final int attackAllIn = 1337;
	
	public static final int commandChannel = 101;
	public static final int commandExpand = 2;
	public static final int commandRally = 3;
	public static final int commandBuildIndividual = 4;
	public static final int commandEnemyNukeHalfDone = 5;
	
	
	public static final int singleExpandXChannel = 102;
	public static final int singleExpandYChannel = 103;
	
	public static final int rallyXChannel = 104;
	public static final int rallyYChannel = 105;
		
	
	public static final int moveTypeChannel = 106;
	public static final int moveTypeRightClick = 1;
	public static final int moveTypeAMove = 2;
	
}
